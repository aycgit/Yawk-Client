package net.yawk.client.gui.screens.mainmenu;
...
public class GuiMainButton extends GuiButton {
	...
	public GuiMainButton(int i, int j, int k, String s)
	{
		this(i, j, k, 120, 20, s);
	}

	public GuiMainButton(int i, int j, int k, int l, int i1, String s)
	{
		super(i, j, k, l, i1, s);
	}
	...
	public void drawButton(Minecraft mc, int mx, int my)
	{
		...
		if (flag)
		{ // Hover Action
			GuiUtils.drawBorderedRect(xPosition, yPosition, xPosition + width, yPosition + height, 1, 0x90FFFFFF, 0x80000000);
			drawCenteredString(fontrenderer, displayString, xPosition + width / 2, yPosition + (height - 8) / 2, 0xFFFFFFFF);
		}
		else { // Normal
			GuiUtils.drawBorderedRect(xPosition, yPosition, xPosition + width, yPosition + height, 1, 0x90FFFFFF, 0x60000000);
